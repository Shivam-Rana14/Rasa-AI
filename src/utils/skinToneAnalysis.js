    ...
    img.onload = async () => {
      try {
        ...
        if (faces.length === 0) {
          ...
          const skinTone = await analyzeSkinToneFromRegion(canvas, {
          ...
          // Use detected face region for skin tone analysis
          const faceBox = faces[0].boundingBox;
          // Focus on middle of the face (forehead area) for more accurate skin tone detection
          const foreheadRegion = {
            x: faceBox.x + faceBox.width * 0.25,
            y: faceBox.y + faceBox.height * 0.15,
            width: faceBox.width * 0.5,
            height: faceBox.height * 0.25
          };
          const skinTone = await analyzeSkinToneFromRegion(canvas, foreheadRegion);
          resolve(skinTone);
        }
      } catch (err) {
        console.error('Error in skin tone analysis:', err);
        reject(err); // Added proper error rejection
      }
    };
...
const detectFaces = async (image) => {
  try {
    ...
    const faces = await extractFaceBoxes(predictions, [image.height, image.width]);
...
const extractFaceBoxes = async (predictions, imgDims) => {
  try {
    ...
    // Handle different prediction formats based on the model
    let facesArray = [];
    
    if (Array.isArray(predictions) && predictions.length >= 2) {
      // Blazeface format: [faceTensor, scoresTensor]
      ...
      // Alternative format or fall back to a simpler approach
      const detections = await predictions.array();
      if (detections && detections.length > 0) {
        // Process whatever format the model returned
        facesArray = detections.map(detection => {
          return {
            boundingBox: {
              x: detection[0] * width,
              y: detection[1] * height,
              width: detection[2] * width,
              height: detection[3] * height
            },
            confidence: detection[4] || 0.8
          };
        });
      }
    }
...
const analyzeSkinToneFromRegion = async (canvas, region) => {
    try {
      ...
      // First validate all inputs are numbers
      if (isNaN(region.x) || isNaN(region.y) || isNaN(region.width) || isNaN(region.height)) {
        throw new Error('Invalid region coordinates - contains NaN values');
      }
      ...
      // Ensure width and height are positive and within canvas bounds
      ...
      // Now safely get image data
      const imageData = ctx.getImageData(x, y, width, height);
      const data = imageData.data;
    
    // Filter out non-skin pixels using improved skin detection algorithm
    const skinPixels = [];
    for (let i = 0; i < data.length; i += 4) {
      ...
      // Skin tone typically falls within specific YCbCr ranges
    ...
    // Use ITA value for more accurate skin tone classification
    const lab = rgbToLab(avgColor.r, avgColor.g, avgColor.b);
    const ita = Math.atan((lab.l - 50) / lab.b) * (180 / Math.PI);
    
    let skinTone;
    if (ita > 55) skinTone = 1; // Very Light
    else if (ita > 41) skinTone = 2; // Light
    else if (ita > 28) skinTone = 3; // Intermediate
    else if (ita > 10) skinTone = 4; // Tan
    else if (ita > -10) skinTone = 5; // Brown
    else if (ita > -30) skinTone = 6; // Dark
    else skinTone = 7; // Very Dark
    
    return skinTone;
  } catch (error) {
    console.error('Error analyzing skin tone from region:', error);
    throw error; // Re-throw any errors that occur
  }
};
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Simple luminance-based classification
